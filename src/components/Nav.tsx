...
import { useColorModeValue, Text, Box, Link } from '@chakra-ui/react';
...
export const Nav: React.FC<NavProps> = ({ to, title, active, children, onClick }) => {
  ...
  const color = '#A6A0BB';
  const activeColor = 'linear-gradient(to right, #6C9EFF, #09CA65 35%, #7BD2A2 70%, #FBFE7F)';

  return (
    <Box as={to ? Link : null} to={to || '#'} p={['6px', '8px']} textDecoration="none"
